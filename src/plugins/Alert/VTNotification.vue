<script>
import { state, methods } from './notifications';

export default {
    data() {
        return {
            notifications: state.alertData,
        };
    },
    // inject: {
    ...
    computed: {
        sortedNotifications() {
            console.log(state.alertGroupData.position);
            if (state.alertGroupData.position === 'bottom') { return [...this.notificationsByGroup].slice(0, this.maxNotifications); }
        ...
        notificationsByGroup() {
            return this.notifications.filter((n) => n.group === state.alertGroupData.group);
        },
    },
    methods: {
        close(id) {
